            ...
            <div>
                ...
                <svg onClick={(e) => { this.props.set_hidden(this.props.index) }} className="marker" xmlns="http://www.w3.org/2000/svg" width="37.5" height="37.5" viewBox="0 0 37.5 37.5" fill="white">
                    ...
                    {colors.map((el,index,arr)=>{
                         ...
                         const pointStart = index / arr.length * Math.PI * 2
                         const pointEnd = pointStart + (1 / arr.length) * Math.PI * 2
                        
                         const d = "M " + (Math.sin(pointStart)*12.5+18.75) + " " + (Math.cos(pointStart)*12.5+18.75) + " a 12.5 12.5 0 0 0 " +(Math.sin(pointEnd)-Math.sin(pointStart))*12.5 + " " + (Math.cos(pointEnd)-Math.cos(pointStart))*12.5

                        return (
                            <path key={index} d={d} stroke={el.color} strokeWidth="3" fill="none" />
                        )
                    })}
            ...
            <div>
                ...
                <div className='plan_container' style={{width: this.props.store.test.width+'px', height:this.props.store.test.height+'px'}}>
                    ...
                    <div className='plan_doc' style={{ position: 'relative', height: this.props.store.test.plan_size.h * this.props.store.test.scale, width: this.props.store.test.plan_size.w * this.props.store.test.scale }}>
                        ...
                        <img onClick={(e) => { this.props.clear_hidden() }} className='plan_img' style={{ height: this.props.store.test.plan_size.h * this.props.store.test.scale, width: this.props.store.test.plan_size.w * this.props.store.test.scale }} src={planes[this.props.store.test.plan_index]} alt="plan"/>
